...
The Port Meridian Ferry System published its autumn timetable on Monday, and the number that matters sits in the third column of the Cordage Island page: eleven round trips a day beginning September 8, down from fourteen through the summer.
...
> A boat that cannot sail is worth less than a boat that sails eleven times.

## The case for cutting anyway

The counter-argument does not come from accountants. It comes from the engine room.
...
Rather than restore sailings it cannot crew, the ferry system is changing how the eleven are sold. Three measures take effect with the new timetable.

First, the reservation window for autumn vehicle slots opens 45 days out rather than 30, which planners expect will spread booking pressure away from the Wednesday panic. Second, the no-show penalty rises from a forfeited $6 reservation fee to a forfeited fee plus a 14-day block on making a new reservation for the same weekday — a response to a no-show rate that reached 22 percent on peak autumn Sundays, meaning roughly one in five reserved deck spaces sailed empty past a standby line.
...
The practical guidance is unromantic. Book vehicle space the day the window opens if your dates are fixed. If your dates are not fixed, travel Tuesday or Wednesday, when autumn deck utilization runs in the low fifties and the boat is genuinely pleasant. Leave the car in the terminal lot and walk on if your destination is South Haven village or the Cordage harbor front, both of which are compact enough to cross in twenty minutes.

And treat the last sailing as a real constraint rather than a suggestion. The 7:40 p.m. departure becomes a 6:15 p.m. departure on September 8, and the gap between those two times has swallowed more autumn dinner reservations than weather ever has.
